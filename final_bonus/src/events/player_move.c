...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/03/22 16:55:48 by aalkhaso          #+#    #+#             */
/*   Updated: 2026/03/22 16:55:48 by aalkhaso         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void     apply_move(t_game *game, double dx, double dy)
{
        ...
        p = &game->player;
        map = game->config.map;
        if (map[(int)p->y][(int)(p->x + dx)] != '1' && !is_closed_door((int)(p->x
                                + dx), (int)p->y, map, game))
                p->x += dx;
        if (map[(int)(p->y + dy)][(int)p->x] != '1' && !is_closed_door((int)p->x,
                        (int)(p->y + dy), map, game))
                p->y += dy;
}
